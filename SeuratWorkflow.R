...
SeuratWorkflow <- function(seurat,
  ...
  if (run_integration && "CCAIntegration" %in% integration_method) {
    Idents(seurat) <- batch_column
    seurat <- seurat %>%
  ...
  if (run_integration && "RPCAIntegration" %in% integration_method) {
    Idents(seurat) <- batch_column
    seurat <- seurat %>%
  ...
  if (run_integration && "jpca" %in% integration_method) {
    Idents(seurat) <- batch_column
    seurat <- seurat %>%
  ...
  if (!run_clustering) {
    ...
    reduction_for_clustering <- ifelse(run_integration, "harmony", "pca") # Default to PCA if no integration is performed
