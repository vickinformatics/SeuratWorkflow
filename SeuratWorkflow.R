...
SeuratWorkflow <- function(seurat,
  ...
  if (run_tSNE_UMAP) {
    ...
    seurat <- seurat %>%
      RunTSNE(reduction = "pca", dims = 1:PCs_pca, reduction.name = "tsne.unintegrated", check_duplicates = check_duplicates) %>%
  ...
  if (run_integration && "harmony" %in% integration_method && run_tSNE_UMAP) {
    Idents(seurat) <- batch_column
    seurat <- seurat %>%
      RunTSNE(reduction = "harmony", dims = 1:PCs_harmony, reduction.name = "tsne.harmony", check_duplicates = check_duplicates) %>%
  ...
  if (run_integration && "CCAIntegration" %in% integration_method && run_tSNE_UMAP) {
    Idents(seurat) <- batch_column
    seurat <- seurat %>%
      RunTSNE(reduction = "cca", dims = 1:PCs_cca, reduction.name = "tsne.cca", check_duplicates = check_duplicates) %>%
  ...
  if (run_integration && "RPCAIntegration" %in% integration_method && run_tSNE_UMAP) {
    Idents(seurat) <- batch_column
    seurat <- seurat %>%
      RunTSNE(reduction = "rpca", dims = 1:PCs_rpca, reduction.name = "tsne.rpca", check_duplicates = check_duplicates) %>%
  ...
  if (run_integration && "JointPCAIntegration" %in% integration_method && run_tSNE_UMAP) {
    Idents(seurat) <- batch_column
    seurat <- seurat %>%
      RunTSNE(reduction = "jpca", dims = 1:PCs_jpca, reduction.name = "tsne.jpca", check_duplicates = check_duplicates) %>%
